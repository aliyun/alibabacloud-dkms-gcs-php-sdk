    ...
    public function doRequest($apiName, $apiVersion, $protocol, $method, $signatureMethod, $reqBodyBytes, $headers, $runtime){
        ...
        while (Tea::allowRetry(@$_runtime["retry"], $_retryTimes, $_now)) {
            ...
            try {
                ...
                $_request->body = Utils::toString($reqBodyBytes);
                $strToSign = AlibabaCloudDkmsGcsOpenApiUtilClient::getStringToSign($_request);
                $_request->headers["authorization"] = $this->_credential->getSignature($strToSign);
                $_lastRequest = $_request;
                if ($runtime->ignoreSSL) {
                    Tea::config(['verify' => false]);
                } else {
                    if (isset($runtime->verify)){
                        Tea::config(['verify' => $runtime->verify]);
                    }
                }
                ...
                if (Utils::is4xx($_response->statusCode) || Utils::is5xx($_response->statusCode)) {
                    ...
                    $respMap = AlibabaCloudDkmsGcsOpenApiUtilClient::getErrMessage($bodyBytes);
                    throw new TeaError([
                        "code" => @$respMap["Code"],
